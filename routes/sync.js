...
router.get('/pull', async (req, res) => {
    try {
        ...
        // Find changes for this organization since last pull
        const productChanges = await Product.find({
            orgId: orgId,
            lastModifiedAt: { $gt: lastPulledAt }
        });
        const logChanges = await InventoryLog.find({
            orgId: orgId,
            lastModifiedAt: { $gt: lastPulledAt }
        });
    ...
    try {
        ...
        if (changes.products) {
            for (const item of changes.products.created || []) {
                await Product.create({ ...item, orgId, lastModifiedAt: now, isDeleted: false });
            }
            for (const item of changes.products.updated || []) {
                await Product.findOneAndUpdate(
                    { wmId: item.wmId || item.id, orgId },
                    { ...item, lastModifiedAt: now, isDeleted: false }
                );
            }
            for (const id of changes.products.deleted || []) {
                await Product.findOneAndUpdate({ wmId: id, orgId }, { isDeleted: true, lastModifiedAt: now });
            }
        }

        if (changes.inventory_logs) {
            for (const item of changes.inventory_logs.created || []) {
                await InventoryLog.create({ ...item, orgId, lastModifiedAt: now, isDeleted: false });
            }
            ...
            for (const id of changes.inventory_logs.deleted || []) {
                await InventoryLog.findOneAndUpdate({ wmId: id, orgId }, { isDeleted: true, lastModifiedAt: now });
            }
        }
